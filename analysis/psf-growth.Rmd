---
title: "Plant-Soil Feedback Greenhouse Experiment - Soil and Growth Results (Dark & Light, with errors)"
...
## Growth Data
Wide format has all of the NAs removed

```{r df, include = FALSE}
# generation 1
growth_data_gen1 <- read_csv(here("data/growth/growth1.csv"))

df1_growth_long <- growth_data_gen1 %>%
  dplyr::select(ID:growth, above, below, biomass) %>%
...
# generation 2
growth_data_gen2 <- read_csv(here("data/growth/growth2.csv"))

df2_growth_long <- growth_data_gen2 %>%
  dplyr::select(ID:growth, above, below, biomass) %>% 
  gather(day, height, week0:week5) 
...
# generation 3
growth_data_gen3 <- read_csv(here("data/growth/growth3.csv"))

df3_growth_long <- growth_data_gen3 %>%
  dplyr::select(ID:growth, above, below, biomass) %>% 
  gather(day, height, week0:week5) 
...
fig_soil_pca <- autoplot(soil_pca,
  ...
  scale_shape_discrete(palette = "Set1", "Invasion status") +
  annotate("text",
...
```{r nmds 1 euclidean}
set.seed(1)
soil_nmds1 <- metaMDS(soilchem_data, dist = "eu", k = 2, trymax = 100)
soil_nmds1
stressplot(soil_nmds1)
```

```{r nmds1 scores}
nmds1_scores <- as.data.frame(scores(soil_nmds1, display = "sites"))
...
```{r nmds}
fig_soil_nmds <- ggplot(data = nmds1_scores, aes(x = NMDS1, y = NMDS2,
...
The two tables below includes the main summary statistics for aboveground and belowground biomass of *L. vulgaris* in generation one.

```{r generation 1 aboveground biomass summary}
rmarkdown::paged_table(df1_above_sum)
```

```{r generation 1 belowground biomass summary}
rmarkdown::paged_table(df1_below_sum)
```

```{r generation 1 total biomass summary}
rmarkdown::paged_table(df1_biomass_sum)
```

```{r generation 1 intervalplot}
...
```{r generation 1 above & below interaction plot}
fig1_above_interaction <- ggplot(df1_growth_wide, aes(x = sterile_status, y = above, colour = invaded_status)) +
  ...
  theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Aboveground biomass (mg) in generation 1")
...
fig1_below_interaction <- ggplot(df1_growth_wide, aes(x = sterile_status, y = below, colour = invaded_status)) +
  ...
  theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Belowground biomass (mg) in generation 1")

fig1_above_interaction
fig1_below_interaction
```

```{r generation 1 all interaction plot}
fig1_biomass_interaction <- df1_growth_wide %>%
  ...
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Total biomass (mg) in generation 1",
...
```{r etasquared above biomass gen1}
etaSquared(abovebiomass_aov, type = 2)
```

η2 for belowground biomass
```{r etasquared below biomass gen1}
...
Anova(fixed_model_1, p.adjust.method = TRUE) # type?
...
The best fixed effects model is the one with initial height as a fixed variable and site must be nested within location as random variables. However, the model without location nesting is also a well-performing model (AICc<2).
...
```{r}
df1_growth_wide$fit_slope <- predict(fixed_model_1)
...
```
None of the fixed effects were significant predictors of biomass in generation one.


Looking at the mixed model as well.
```{r}
summary(mixed_model_1)
summary(update(mixed_model_1))
...
```{r generation 2 above & below interaction plot}
fig2_above_interaction <- ggplot(df2_growth_wide, aes(x = sterile_status, y = above, colour = invaded_status)) +
  geom_point() +
  stat_summary(fun = mean, aes(group = invaded_status), geom = "line") +
  # theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Aboveground biomass (mg) in generation 2")
...
fig2_below_interaction <- ggplot(df2_growth_wide, aes(x = sterile_status, y = below, colour = invaded_status)) +
  ...
  # theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Belowground biomass (mg) in generation 2")

fig2_above_interaction
fig2_below_interaction
```

```{r generation 2 all interaction plot}
fig2_biomass_interaction <- df2_growth_wide %>%
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.1) +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Total biomass (mg) in generation 2",
       colour = "Invaded Status") +
  # theme_vz() +
  theme(legend.position = "right")
...
η2 for aboveground biomass
```{r etasquared above biomass gen2}
etaSquared(abovebiomass_aov, type = 2)
```

η2 for belowground biomass
```{r etasquared below biomass gen2}
etaSquared(belowbiomass_aov, type = 2)
```
...
```{r generation 3 aboveground biomass summary}
rmarkdown::paged_table(df3_above_sum)
```

```{r generation 3 belowground biomass summary}
rmarkdown::paged_table(df3_below_sum)
```

```{r generation 3 total biomass summary}
rmarkdown::paged_table(df3_biomass_sum)
```

```{r generation 3 intervalplot}
...
Interval plots show that there is lower biomass in live, invaded soils compared to other three treatments. The standard error bars overlap much less in the third generation.

```{r generation 3 above & below interaction plot}
fig3_above_interaction <- ggplot(df3_growth_wide, aes(x = sterile_status, y = above, colour = invaded_status)) +
  geom_point() +
  stat_summary(fun = mean, aes(group = invaded_status), geom = "line") +
  # theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Aboveground biomass (mg) in generation 3")
...
fig3_below_interaction <- ggplot(df3_growth_wide, aes(x = sterile_status, y = below, colour = invaded_status)) +
  ...
  # theme_vz() +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Belowground biomass (mg) in generation 3")

fig3_above_interaction
fig3_below_interaction
```

```{r generation 3 all interation plot}
fig3_biomass_interaction <- df3_growth_wide %>%
  ...
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.1) +
  scale_colour_brewer(palette = "Set1") +
  labs(x = "Sterile Status", y = "Total biomass (mg) in generation 3",
       colour = "Invaded Status") +
  # theme_vz() +
  theme(legend.position = "right")
...
```{r ANOVA below biomass gen3}
belowbiomass_aov <- lm(below ~ invaded_status * sterile_status, data = df3_growth_wide)
belowbiomass_Anova <- Anova(belowbiomass_aov, type = "2")
belowbiomass_Anova
```

The interaction term between `invaded_status` and `sterile_status` was a significant predictor of aboveground biomass, and both `sterile_status` and the interaction were a sigbnificant predictor of belowground biomass. Thus, in generation 3, I can reject the null hypothesis that there are no significant differences between treatment groups.
...
Sterility was a significant predictor of biomass, in which sterile soils resulted in greater biomass. The initial sterilization removed any soil biota that may limit *Linaria* growth. Thus, throughout the two previous generations, individuals were modifying the soil to their benefit in sterile soil, and this inocula resulted in increased growth.

In the `invaded_status` treatment, half of these soils were sterilized prior to Generation 1. However, it appears that the effect of invasion was beginning to swamp out the effect of the initial sterilization. This was not expected, as we hypothesized that individuals in uninvaded sites would be smaller, as they are limited by the soil biota. In actuality, larger individuals resulted from soil inocula that were originally collected from uninvaded sites. This may be because previous generations have modified the uninvaded live and sterile soil. This result may be mostly attributed to the fact that biomass was visibly reduced in live, invaded soils, and, in comparison, both sterile treatments resulted in a higher biomass.

**net effect - less pathogens, more mutalists?**

Furthermore, the interaction between `sterile_status` and `invaded_status` was a significant predictor of biomass: uninvaded, sterile soils resulted in reduced biomass. This appears to be further evidence of negative plant-soil feedback. *Linaria* negatively alter the soil that reduces growth of future generations, even in sterile soil originally collected from uninvaded sites. It's possible that the uninvaded soils were originally unsuitable for *Linaria*, but after sterilization, it is assumed that any pathogens in the soil would be removed.
...
df_all_long$generation <- as.factor(df_all_long$generation)

# add columns for
# sterile status & generation and 
# invaded status & generation
df_all_long$sterile_generation <- paste(df_all_long$generation,
                                        df_all_long$sterile_status, 
                                        sep = "_")

df_all_long$invaded_generation <- paste(df_all_long$generation,
                                        df_all_long$invaded_status, 
                                        sep = "_")

df_all_long <- df_all_long %>% 
  filter(!is.na(biomass) & !is.na(invaded_status))
```


### Summary
```{r}
df_biomass_sum <- df_all_long %>% 
  filter(!is.na(biomass)) %>% 
  group_by(generation, sterile_status, invaded_status) %>% 
  summarise(
    count = n(),
    mean = mean(biomass*1000, na.rm = TRUE),
    median = median(biomass*1000, na.rm = TRUE),
    IQR = IQR(biomass*1000, na.rm = TRUE),
    sd = sd(biomass*1000),
    se = sd / sqrt(count)
  )
...
```{r}
# generation labels
gen_labs <- c("Generation 1", "Generation 2", "Generation 3")
names(gen_labs) <- c("1", "2", "3")
```
...
figall_biomass_interaction <- ggplot(df_biomass_sum,
  ...
  facet_wrap(~ generation,
             labeller = labeller(generation = gen_labs)) +
